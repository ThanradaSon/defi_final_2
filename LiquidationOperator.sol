...
contract LiquidationOperator is IUniswapV2Callee {
    ...
    IUniswapV2Pair immutable uniswapV2Pair_WBTC_USDT; //////ลองเพิ่ม
    ...
    function operate() external {
        ...
        uniswapV2Pair_WBTC_USDT.swap(0, debt_USDT, address(this), "$"); // กู้usdtมา
    ...
    function uniswapV2Call(
        ...
        // 2.1 liquidate the target user ใช้หนี้
        
        uint debtToCover = amount1;
        USDT.approve(address(lendingPool), debtToCover); 
        lendingPool.liquidationCall(address(WBTC), address(USDT), liquidationTarget, debtToCover, false); //จะจ่ายเท่าไร
        uint collateral_WBTC = WBTC.balanceOf(address(this)); // ได้ wbtc ที่ค้ำประกันออกมา
        ...
        // 2.2 swap WBTC for other things or repay directly
        collateral_WBTC = WBTC.balanceOf(address(this));
